...
export default function AdminSponsorsPage() {
  ...
  if (loading) content = <LoadingState />;
    ...
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm"><div className="overflow-x-auto"><table className="w-full min-w-[1180px] text-right text-sm">
      <thead className="bg-[#F5F7FA] text-[#374151]"><tr><th className="px-4 py-3 font-extrabold">الاسم</th><th className="px-4 py-3 font-extrabold">البريد الإلكتروني</th><th className="px-4 py-3 font-extrabold">الهاتف</th><th className="px-4 py-3 font-extrabold">الموقع</th><th className="px-4 py-3 font-extrabold">الحساب</th><th className="px-4 py-3 font-extrabold">عدد الكفالات</th><th className="px-4 py-3 font-extrabold">الإجراءات</th></tr></thead>
      <tbody className="divide-y divide-gray-100">{filteredSponsors.map((sponsor) => <tr key={sponsor.sponsorId} className="hover:bg-gray-50/70"><td className="px-4 py-4 font-bold text-[#003469]">{sponsor.fullName || "—"}</td><td className="px-4 py-4 text-gray-600">{sponsor.email || "—"}</td><td dir="ltr" className="px-4 py-4 text-right text-gray-600">{sponsor.phoneNumber || "—"}</td><td className="px-4 py-4">{[sponsor.city, sponsor.country].filter(Boolean).join("، ") || "—"}</td><td className="px-4 py-4"><span className={`rounded-full px-3 py-1 text-xs font-bold ${accountStatusClasses(sponsor.accountStatus)}`}>{accountStatusLabel(sponsor.accountStatus)}</span></td><td className="px-4 py-4 font-extrabold text-[#0D4B8E]">{sponsor.totalSponsorships ?? 0}</td><td className="px-4 py-4"><div className="flex flex-wrap gap-2"><button type="button" disabled={Boolean(busy)} onClick={() => openSponsor(sponsor.sponsorId)} className="inline-flex items-center gap-1 rounded-md bg-[#E8F1FA] px-3 py-2 text-xs font-bold text-[#0D4B8E] disabled:opacity-50"><FiEye />عرض التفاصيل</button><button type="button" disabled={Boolean(busy)} onClick={() => openSponsor(sponsor.sponsorId, "edit")} className="inline-flex items-center gap-1 rounded-md border border-gray-300 px-3 py-2 text-xs font-bold text-gray-700 disabled:opacity-50"><FiEdit2 />تعديل</button>{sponsor.canSuspend && <button type="button" disabled={Boolean(busy)} onClick={() => showStatusConfirmation(sponsor, false)} className="inline-flex items-center gap-1 rounded-md bg-amber-600 px-3 py-2 text-xs font-bold text-white disabled:opacity-50"><MdPauseCircleOutline />تعليق الحساب</button>}{sponsor.canReactivate && <button type="button" disabled={Boolean(busy)} onClick={() => showStatusConfirmation(sponsor, true)} className="inline-flex items-center gap-1 rounded-md bg-emerald-600 px-3 py-2 text-xs font-bold text-white disabled:opacity-50"><MdPlayCircleOutline />إعادة تفعيل الحساب</button>}<button type="button" disabled={Boolean(busy)} onClick={() => openDelete(sponsor.sponsorId)} className="inline-flex items-center gap-1 rounded-md border border-red-200 px-3 py-2 text-xs font-bold text-red-700 disabled:opacity-50"><FiTrash2 />حذف نهائي</button></div></td></tr>)}</tbody>
    </table></div></div>
  );
