      ...
      <div className="overflow-x-auto"><table className="w-full min-w-[1080px] text-right text-xs">
        ...
        <tbody className="divide-y divide-gray-100">{filteredGuardians.map((guardian) => (
          <tr key={guardian.guardianId} className="hover:bg-gray-50/70">
            <td title={guardian.fullName || undefined} className="max-w-[190px] px-3 py-3 font-bold text-[#003469]"><div className="flex min-w-0 items-center gap-2"><AdminEntityAvatar name={guardian.fullName} hasImage={guardian.hasProfileImage} imageEndpoint={`/api/v1/admin/guardians/${guardian.guardianId}/profile-image`} alt={`صورة الوصي ${guardian.fullName || ""}`.trim()} size="sm" /><span className="truncate">{guardian.fullName || "—"}</span></div></td><td title={guardian.email || undefined} className="max-w-[210px] truncate px-3 py-3 text-[11px] text-gray-600">{guardian.email || "—"}</td><td dir="ltr" className="whitespace-nowrap px-3 py-3 text-right text-[11px] text-gray-600">{guardian.phoneNumber || "—"}</td><td title={[guardian.city, guardian.country].filter(Boolean).join("، ") || undefined} className="max-w-[140px] truncate px-3 py-3">{[guardian.city, guardian.country].filter(Boolean).join("، ") || "—"}</td><td className="whitespace-nowrap px-3 py-3"><VerificationStatusBadge status={guardian.verificationStatus} /></td><td className="whitespace-nowrap px-3 py-3"><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${accountStatusClasses(guardian.accountStatus)}`}>{accountStatusLabel(guardian.accountStatus)}</span></td><td className="max-w-[130px] truncate px-3 py-3">{guardian.hasFamily ? localizeStatus(guardian.familyStatus) : "لا توجد عائلة"}</td>
    ...
    {selected && dialogMode === "details" && <AdminDialog title="تفاصيل الوصي" size="max-w-5xl" onClose={() => { setSelected(null); setDialogMode(""); }} footer={<>
      ...
      <div className="space-y-5">
        <AdminDetailsHero icon={HiOutlineIdentification} eyebrow="ملف الوصي" title={selected.fullName} subtitle={selected.email} badges={[{ label: "التحقق", value: localizeVerificationStatus(selected.verificationStatus) }, { label: "الحساب", value: accountStatusLabel(selected.accountStatus) }]}>
          <AdminEntityAvatar name={selected.fullName} hasImage={selected.hasProfileImage} imageEndpoint={`/api/v1/admin/guardians/${selected.guardianId}/profile-image`} alt={`صورة الوصي ${selected.fullName || ""}`.trim()} size="hero" />
        </AdminDetailsHero>
