...
export default function AdminGuardianDocumentsReviewPage() {
  ...
  }, [debouncedSearch, documentTypeFilter, guardianId, page, pageSize, reviewedFrom, reviewedTo, statusFilter, uploadedFrom, uploadedTo]);
  ...
  const hasActiveFilters = Boolean(searchInput.trim() || statusFilter || documentTypeFilter || guardianId || uploadedFrom || uploadedTo || reviewedFrom || reviewedTo);
  ...
  const clearFilters = () => {
    ...
    setPage(1);
    setGuardianId(""); setUploadedFrom(""); setUploadedTo(""); setReviewedFrom(""); setReviewedTo("");
  };
    ...
    <AdminLayout title="وثائق الأوصياء">
      <div className="space-y-6" dir="rtl">
        ...
        <div className="grid gap-3 rounded-xl border border-gray-200 bg-white p-4 sm:grid-cols-2 xl:grid-cols-4">
          <label className="relative"><span className="sr-only">البحث في وثائق الأوصياء</span><FiSearch className="absolute right-3 top-3 text-gray-400" aria-hidden="true" /><input value={searchInput} onChange={(event) => { setSearchInput(event.target.value); setPage(1); }} placeholder="ابحث باسم الوصي أو البريد أو الهاتف أو رقم الهوية..." className="w-full rounded-lg border border-gray-300 py-2.5 pr-10 pl-3 text-sm outline-none focus:border-[#0D4B8E]" /></label>
          ...
          <select value={documentTypeFilter} onChange={(event) => { setDocumentTypeFilter(event.target.value ? Number(event.target.value) : ""); setPage(1); }} aria-label="تصفية حسب نوع الوثيقة" className="rounded-lg border border-gray-300 px-3 py-2.5 text-sm">{GUARDIAN_DOCUMENT_TYPE_FILTERS.map((filter) => <option key={filter.label} value={filter.value}>{filter.label}</option>)}</select>
