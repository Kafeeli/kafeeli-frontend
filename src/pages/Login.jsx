...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (
        serverMessage.includes("تأكيد بريدك") ||
        serverMessage.includes("تأكيد البريد") ||
        serverMessage.includes("verify") ||
        (serverMessage.toLowerCase().includes("email") && serverMessage.toLowerCase().includes("confirm"))
      ) {
        localStorage.setItem("pendingVerificationEmail", email.trim());
        navigate(`/invalid-email?email=${encodeURIComponent(email)}`);
        return;
      }

      setErrorMessage(serverMessage || "حدث خطأ أثناء تسجيل الدخول، حاول مرة أخرى");
    } finally {
      setIsSubmitting(false);
    }
  };
