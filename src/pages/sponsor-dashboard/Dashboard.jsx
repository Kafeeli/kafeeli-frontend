...
function AccountCard({ icon, label, value }) {
  return (
    <div className="flex items-center gap-3 rounded-xl border border-gray-300 bg-white p-5">
      <div className="grid h-12 w-12 place-items-center rounded-full bg-blue-50 text-xl text-[#0D4B8E]">
        {icon}
      </div>
      <div>
        <p className="text-sm text-gray-500">{label}</p>
        <p className="mt-1 font-bold text-[#003469]">{value}</p>
      </div>
    </div>
  );
}
...
export default function Dashboard() {
  ...
  const loadSponsorships = useCallback(async () => {
    ...
    try {
      ...
      setSponsorships(
        Array.isArray(sponsorshipData?.sponsorships)
          ? sponsorshipData.sponsorships
          : [],
      );
    } catch (requestError) {
      setSponsorshipsError(
        getApiErrorMessage(requestError, "تعذر تحميل الكفالات."),
      );
    } finally {
        ...
        <main className="flex-1 space-y-6 p-4 sm:p-6">
          ...
          {!loading && error && (
            <div className="rounded-xl border border-red-200 bg-red-50 p-8 text-center text-red-700">
              <FiAlertCircle className="mx-auto mb-3 text-3xl" />
              <p>{error}</p>
              <button
                type="button"
                onClick={retry}
                className="mt-4 rounded-lg bg-[#003469] px-5 py-2 font-bold text-white"
              >
                إعادة المحاولة
              </button>
            </div>
          ...
          {!loading && !error && data && (
            <>
              {data.nextRequiredActionMessage && (
              ...
              <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
                <AccountCard
                  icon={<FaEnvelope />}
                  label="حالة البريد"
                  value={data.emailConfirmed ? "مؤكد" : "غير مؤكد"}
                />
                <AccountCard
                  icon={<FaShieldAlt />}
                  label="حالة الحساب"
              ...
              {data.activeFamiliesAvailableCount != null && (
                <section className="rounded-xl border border-[#C2C6D2] bg-white p-5">
                  ...
                  <p className="text-3xl font-extrabold text-[#003469]">
                    {data.activeFamiliesAvailableCount}
                  </p>
                  <p className="mt-1 text-sm text-gray-500">
                    العائلات النشطة المتاحة
                  </p>
                  <Link
                    to="/sponsor/families"
                    className="mt-4 inline-block font-bold text-[#0F7F8C] hover:underline"
                  >
                    تصفح العائلات
                  </Link>
                </section>
          ...
          <section className="overflow-hidden rounded-xl border border-[#C2C6D2] bg-white">
            <div className="flex items-center justify-between px-4 py-3">
              <h2 className="font-bold text-gray-800">أحدث كفالاتي</h2>
              <Link
                to="/sponsor/sponsorships"
                className="text-sm font-bold text-[#0F7F8C] hover:underline"
              >
                عرض الكل
              </Link>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-sm">
                <thead>
                  <tr className="bg-[#F5F6F8] text-xs text-gray-600">
                    <th className="px-4 py-3 text-right">المستفيد</th>
                    <th className="px-4 py-3 text-right">نوع الكفالة</th>
                  ...
                  {sponsorships.slice(0, 5).map((row) => (
                    ...
                    >
                      ...
                      <td className="whitespace-nowrap px-4 py-3">
                        <span className="rounded-full bg-[#E8F1FA] px-3 py-1 text-xs text-[#0D4B8E]">
                          {localizeStatus(row.statusLabel || row.status)}
                        </span>
                      </td>
                      <td className="whitespace-nowrap px-4 py-3 text-gray-600">
                      ...
                      <td className="whitespace-nowrap px-4 py-3">
                        <Link
                          to={`/sponsor/sponsorships/${row.sponsorshipId}`}
                          className="inline-flex items-center gap-1 text-xs font-bold text-[#0F7F8C] hover:underline"
                        >
                          التفاصيل <FiEye />
                        </Link>
                      </td>
              ...
              {!sponsorshipsLoading && sponsorshipsError && (
                <div className="p-6 text-center text-sm text-red-600">
                  <p>{sponsorshipsError}</p>
                  <button
                    type="button"
                    onClick={loadSponsorships}
                    className="mt-2 font-bold text-[#0F7F8C] hover:underline"
                  >
                    إعادة المحاولة
                  </button>
                </div>
