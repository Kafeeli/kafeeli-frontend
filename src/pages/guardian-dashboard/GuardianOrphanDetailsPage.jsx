  ...
  if (loading) content = <LoadingState count={3} />;
    ...
    content = (
      <div className="space-y-6">
        <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">
                  {orphan.headOfHouseholdName || "—"}
                </p>
              </div>
            </div>
            <span className="rounded-full bg-[#E8F1FA] px-3 py-1 text-sm font-bold text-[#0D4B8E]">
              {orphan.orphanStatus || "—"}
            </span>
          </div>
          ...
          <dl className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div>
              ...
              <dd className="font-bold">{orphan.maskedNationalId || "—"}</dd>
            </div>
            <div>
              <dt className="text-sm text-gray-500">العمر</dt>
              <dd className="font-bold">{orphan.age}</dd>
            </div>
            <div>
              <dt className="text-sm text-gray-500">الجنس</dt>
              <dd className="font-bold">{orphan.gender || "—"}</dd>
            </div>
            <div>
              <dt className="text-sm text-gray-500">التعليم</dt>
              <dd className="font-bold">{orphan.educationalStatus || "—"}</dd>
            </div>
          </dl>
          <div className="mt-5 flex flex-wrap gap-3">
            {orphan.canEdit && (
              <Link
                ...
                className="inline-flex items-center gap-2 rounded-lg bg-[#0D4B8E] px-4 py-2.5 text-sm font-bold text-white"
              >
                <MdEdit />
                تعديل البيانات
              </Link>
            ...
            {orphan.canResubmit && (
              <button
                ...
                onClick={resubmit}
                className="rounded-lg bg-[#008C78] px-4 py-2.5 text-sm font-bold text-white"
              >
                {busy === "resubmit"
                  ? "جارٍ الإرسال..."
        ...
        <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <h2 className="flex items-center gap-2 text-lg font-extrabold text-[#003469]">
            <MdDescription />
            وثائق اليتيم
          </h2>
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            {documents.map((document) => (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h3 className="font-bold">
                      {document.arabicLabel || document.documentType || "—"}
                    </h3>
                    <p className="mt-1 text-xs text-gray-500">
                      {document.verificationStatus ||
                        (document.hasCurrentDocument ? "مرفوعة" : "غير مرفوعة")}
                    </p>
                  </div>
                  {document.isRequired && (
                ...
                {document.needsUpdateReason && (
                  <p className="mt-3 rounded bg-amber-50 p-2 text-xs text-amber-800">
                    {document.needsUpdateReason}
                  </p>
                )}
                <div className="mt-3 flex flex-wrap gap-2">
                  {document.canView && document.documentId && (
                    <button
                      type="button"
                      disabled={busy === document.documentId}
                      onClick={() => viewFile(document.documentId)}
                      className="rounded bg-[#E8F1FA] px-3 py-2 text-xs font-bold text-[#0D4B8E]"
                    >
                      عرض الملف
                    </button>
                  )}
                  {document.canReupload && (
                    <label className="cursor-pointer rounded bg-[#008C78] px-3 py-2 text-xs font-bold text-white">
                      {busy === document.documentType
                        ? "جارٍ الرفع..."
                        : "إعادة رفع"}
                      <input
                        type="file"
          ...
          {!documents.length && (
            <p className="mt-4 text-sm text-gray-500">لا توجد وثائق مطلوبة.</p>
          )}
          {actionError && (
            <p
              role="alert"
