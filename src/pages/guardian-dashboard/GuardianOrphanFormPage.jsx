...
const EMPTY_FORM = {
  firstName: "", fatherName: "", grandfatherName: "", nationalId: "",
  dateOfBirth: "", gender: "", educationStatus: "", caseDescription: "",
  profileImage: null, birthCertificate: null,
};
...
export default function GuardianOrphanFormPage({ mode }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (data.hasProfileImage === true) {
        try {
          const objectUrl = URL.createObjectURL(await orphanApi.getProfileImage(orphanId));
  ...
  return (
    <GuardianFlowLayout title={editing ? "تعديل بيانات اليتيم" : "إضافة يتيم"} description={editing ? "تعديل بيانات اليتيم المسجلة" : "إضافة يتيم إلى العائلة النشطة"}>
      <form onSubmit={submit} className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        <div className="grid gap-5 md:grid-cols-3">
          {[["firstName", "الاسم الأول", 150], ["fatherName", "اسم الأب", 150], ["grandfatherName", "اسم الجد", 150], ["nationalId", "رقم الهوية", 9], ["educationStatus", "الحالة التعليمية", 100]].map(([name, label, maxLength]) => (
            <label key={name} className="text-sm font-bold">{label}<input name={name} value={form[name]} onChange={change} required maxLength={maxLength} minLength={name === "nationalId" ? 9 : undefined} className="mt-2 w-full rounded-lg border border-gray-300 px-3 py-2" /></label>
          ))}
          ...
          <div className="text-sm font-bold">
            ...
            {editing && <div className="mt-2 overflow-hidden rounded-xl border border-gray-200 bg-gray-50">
              {previewUrl ? <img src={previewUrl} onError={() => { if (selectedImageUrl) setSelectedImageUrl(""); else setExistingImageUrl(""); }} alt="الصورة الشخصية لليتيم" className="h-48 w-full object-cover" /> : <div className="flex h-48 flex-col items-center justify-center gap-2 text-gray-400" role="img" aria-label="لا توجد صورة شخصية"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="h-12 w-12"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.5 20.25a7.5 7.5 0 0 1 15 0" /></svg><span className="text-xs font-medium">لا توجد صورة متاحة</span></div>}
            </div>}
            <input id="profileImage" type="file" name="profileImage" accept="image/*" onChange={change} className="mt-2 w-full rounded-lg border border-gray-300 px-3 py-2" />
            {editing && <span className="mt-2 block text-xs font-normal text-gray-500">اترك الحقل فارغًا للاحتفاظ بالصورة الحالية.</span>}
          </div>
        ...
        <button type="submit" disabled={submitting} className="mt-5 rounded-lg bg-[#008C78] px-6 py-3 font-bold text-white disabled:bg-gray-400">{submitting ? "جارٍ الحفظ..." : "حفظ وإرسال للمراجعة"}</button>
      </form>
    </GuardianFlowLayout>
  );
}
