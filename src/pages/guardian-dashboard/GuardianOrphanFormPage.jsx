...
export default function GuardianOrphanFormPage({ mode }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setForm({
        ...
        gender:
          data.gender === "Male" ? "1" : data.gender === "Female" ? "2" : "",
  ...
  return (
    <GuardianFlowLayout
      title={editing ? "تعديل بيانات اليتيم" : "إضافة يتيم"}
      description={
        editing
          ? "يجب إعادة إدخال رقم الهوية الكامل؛ يعرض الخادم الرقم المقنّع فقط."
          : "إضافة يتيم إلى العائلة النشطة"
      }
    >
      <form
        onSubmit={submit}
      ...
      >
        <div className="grid gap-5 md:grid-cols-3">
          {[
            ["firstName", "الاسم الأول", 150],
            ["fatherName", "اسم الأب", 150],
            ["grandfatherName", "اسم الجد", 150],
            ["nationalId", "رقم الهوية", 9],
            ["educationStatus", "الحالة التعليمية", 100],
            ...
            <label key={name} className="text-sm font-bold">
              ...
              <input
                ...
                required
                maxLength={maxLength}
                minLength={name === "nationalId" ? 9 : undefined}
        ...
        <button
          disabled={submitting}
          className="mt-5 rounded-lg bg-[#008C78] px-6 py-3 font-bold text-white disabled:bg-gray-400"
        >
