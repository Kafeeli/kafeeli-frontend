...
export default function GuardianOrphansPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data = unwrapResult(
        await orphanApi.getMine(),
        "تعذر تحميل الأيتام.",
      );
      setOrphans(
        (data?.orphans || []).map((item) =>
          localizeDisplayFields(item, ["orphanStatus"]),
        ),
      );
    } catch (requestError) {
  ...
  if (loading) content = <LoadingState />;
    ...
    content = (
      <EmptyState
        icon={MdChildCare}
        title="لا يوجد أيتام مسجلون"
        description="يمكن إضافة يتيم من صفحة عائلة نشطة."
      />
    );
  else
    content = (
        ...
        {orphans.map((orphan) => (
          ...
          >
            <div className="flex items-start justify-between">
              <MdChildCare className="text-3xl text-[#0D4B8E]" />
              <span className="rounded-full bg-[#E8F1FA] px-3 py-1 text-xs font-bold text-[#0D4B8E]">
                {orphan.orphanStatus || "—"}
              </span>
            </div>
            <h2 className="mt-4 text-lg font-extrabold text-[#003469]">
              {orphan.fullName || "—"}
            </h2>
            ...
            <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
              <div>
                <dt className="text-gray-500">العمر</dt>
            ...
            <Link
              to={`/guardian/orphans/${orphan.orphanId}`}
              className="mt-5 block rounded-lg bg-[#0D4B8E] px-4 py-2.5 text-center text-sm font-bold text-white"
            >
              عرض التفاصيل
            </Link>
          </article>
  ...
  return (
    <GuardianFlowLayout
      title="الأيتام"
      description="متابعة حالات الأيتام والوثائق المطلوبة"
    >
      {content}
    </GuardianFlowLayout>
  );
