    ...
    <div className="flex min-h-screen bg-gray-50" dir="rtl">
      ...
      <div className="flex min-w-0 flex-1 flex-col lg:mr-64">
        ...
        <main className="flex-1 p-4 sm:p-6">
          ...
          {!loading && error && (
            <div className="mt-6 rounded-xl border border-red-200 bg-red-50 p-8 text-center text-red-700">
              ...
              <section className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
                {data.verificationStatus && (
                  <StatCard
                    icon={<MdVerifiedUser />}
                    label="حالة التحقق"
                    value={localizeStatus(data.verificationStatus)}
                  />
                )}
                {data.documentsUploadedCount != null && (
                  <StatCard
                    icon={<MdDescription />}
                    label="الوثائق المرفوعة"
                    value={data.documentsUploadedCount}
                  />
                )}
                {data.pendingDocumentsCount != null && (
                  <StatCard
                    icon={<FiCalendar />}
                    label="الوثائق قيد المراجعة"
                    value={data.pendingDocumentsCount}
                  />
                )}
                {data.approvedDocumentsCount != null && (
                  <StatCard
                    icon={<FiCheckCircle />}
                    label="الوثائق المقبولة"
                    value={data.approvedDocumentsCount}
                  />
                )}
                {data.rejectedDocumentsCount != null && (
                  <StatCard
                    icon={<FiAlertCircle />}
                    label="الوثائق المرفوضة"
                    value={data.rejectedDocumentsCount}
                  />
                )}
                {data.familiesCount != null && (
                  <StatCard
                ...
                <dl className="mt-4 grid gap-4 text-sm sm:grid-cols-2 lg:grid-cols-4">
                  <div>
                    ...
                    <dd className="mt-1 font-bold">
                      {data.emailConfirmed ? "مؤكد" : "غير مؤكد"}
                    </dd>
                  </div>
                  <div>
                  ...
                  <div>
                    <dt className="text-gray-500">اكتمال الملف الشخصي</dt>
                    <dd className="mt-1 font-bold">
                      {data.profileCompletionPercentage}%
                    </dd>
                  </div>
                  <div>
                    <dt className="text-gray-500">تاريخ الانضمام</dt>
                    <dd className="mt-1 font-bold">
                      {formatDate(data.joinedAt)}
                    </dd>
                  </div>
                  {data.canManageFamilies != null && (
                    <div>
                      <dt className="text-gray-500">إدارة العائلات</dt>
                      <dd className="mt-1 font-bold">
                        {data.canManageFamilies ? "متاحة" : "غير متاحة حالياً"}
                      </dd>
                    </div>
                  )}
                </dl>
